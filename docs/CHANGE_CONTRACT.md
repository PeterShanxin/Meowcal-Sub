...
- put the release notes (`docs/releases/vX.Y.Z.md`) and the download links in
  `README.md` and `README.zh-CN.md` in a separate `docs` commit in the same
  pull request, so the version commit stays reviewable on its own and the
  published notes land with it;
