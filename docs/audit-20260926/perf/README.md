...
The browser scripts use Playwright's bundled Chromium; set `CHROMIUM_PATH` to
run another build.
...
discarded as browser warm-up. The committed results keep each case's summary
statistics; per-run samples are dropped.
...
`encode-all-variants.json`, `render-variants.json` and `render-rgba-fast.json`
cover the variants that were not kept:

- **JPEG quality 85** - a 17x smaller payload on the 4K frame and the fastest
  render, but lossy on the image users align the subtitle box against, slower
  to encode than PNG `Fast` at 1080p (65 against 21 ms), and slower end to end
  on the flat desktop (115 against 95 ms).
- **`Compression::Fastest`** - encodes about as fast as `Fast`, writes files
  about 6% larger, and renders no faster.
- **RGB with the default preset** - still 400 ms at 1080p and 1.5 s at 4K.
