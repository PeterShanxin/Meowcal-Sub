// Windows does not tie child lifetime to parent lifetime. The Core process owns
// one kill-on-close job object so a managed engine cannot survive a crash,
// forced termination, logoff, or shutdown. Explicit shutdown covers the
// graceful path; the job object covers paths where cleanup code cannot run.
...
mod windows_impl {
    ...
    fn job_handle() -> Result<HANDLE, String> {
    ...
    unsafe fn create_job() -> Result<isize, String> {
        ...
        if let Err(error) = result {
            ...
            return Err(format!("ENGINE_PROCESS_LIFETIME_ARM: {error}"));
        }
    ...
    ///
    /// Starting an engine without this ownership guarantee could strand a
    /// multi-gigabyte process after a crash, so the caller must stop the child
    ...
    pub fn attach_to_app_lifetime(child: &Child) -> Result<(), String> {
        ...
        unsafe { AssignProcessToJobObject(job, process) }
            .map_err(|error| format!("ENGINE_PROCESS_LIFETIME_ATTACH: {error}"))
    }
    ...
    /// looked can free its PID onto an unrelated process before we act on it.
    ///
