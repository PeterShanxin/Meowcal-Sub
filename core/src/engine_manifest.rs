...
pub struct EngineManifest {
    ...
    /// Legacy schema name for the first desktop app version that shipped these
    /// artifacts. Core compatibility is enforced by the protocol handshake.
...
pub struct AuthenticityPolicy {
    /// `embeddedApplicationRelease` is retained as the v1 wire value. The
    /// manifest is now embedded in the versioned Core package.
    pub mode: String,
...
// authenticity model: the manifest is embedded per Core release, so no older
...
impl EngineManifest {
    ...
    fn validate(&self) -> Result<(), ManifestError> {
        ...
        {
            return invalid("launch policy overrides Core-owned launch configuration");
        }
