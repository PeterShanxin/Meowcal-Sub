...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn snapshot_reports_the_build_that_cmd_ver_reports() -> Result<(), String> {
        // `ver` prints "Microsoft Windows [<label> 10.0.<build>.<revision>]",
        // with the label localized, so only the bracketed number is read.
        let output = std::process::Command::new("cmd")
            .args(["/d", "/c", "ver"])
            .output()
            .map_err(|error| format!("could not run ver: {error}"))?;
        let text = String::from_utf8_lossy(&output.stdout);
        let expected = text
            .split(['[', ']'])
            .nth(1)
            .and_then(|bracket| bracket.split_whitespace().last())
            .and_then(|version| version.split('.').nth(2))
            .and_then(|build| build.parse::<u32>().ok())
            .ok_or_else(|| format!("unexpected ver output: {text}"))?;

        let snapshot = snapshot(&std::env::temp_dir())?;
