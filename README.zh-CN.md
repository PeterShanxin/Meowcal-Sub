...
**Windows 11 公开测试版 · v0.8.0** — [更新说明与全部安装文件](https://github.com/PeterShanxin/Meowcal-Sub/releases/tag/v0.8.0)。

| 你的电脑 | 安装包 |
| --- | --- |
| Intel / AMD Windows 电脑 | **[下载 x64（.exe）](https://github.com/PeterShanxin/Meowcal-Sub/releases/download/v0.8.0/Meowcal.Sub_0.8.0_x64-setup.exe)** |
| 骁龙或其他 Windows on ARM 电脑 | **[下载 ARM64（.exe）](https://github.com/PeterShanxin/Meowcal-Sub/releases/download/v0.8.0/Meowcal.Sub_0.8.0_arm64-setup.exe)** |
...
2. **圈选字幕。** 点击 **Select subtitle area**，圈住原字幕，而不是整个视频。
   保持该区域可见；移动视频或改变画面大小后，重新圈选。
...
**也有边界。** 识别效果取决于原文语言、文字清晰度，以及屏幕捕获实际能看到什么。
艺术字体、快速变化的文字或受保护的视频，可能造成漏识别或错误。
翻译并非瞬时完成，也不保证完全准确；速度取决于电脑和文本。
GPU 加速只覆盖已验证的硬件／驱动组合，并有 CPU 回退，不代表支持所有 GPU。
