...
impl ScreenCaptureSession {
    ...
    pub fn new() -> Result<Self, CaptureError> {
        ...
        let d3d_context = unsafe {
            d3d_device.GetImmediateContext()
        ...
        let item_size = item.Size()
            .map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to get item size: {}", e)))?;
        ...
        let device = d3d::create_direct3d_device(&d3d_device)
            .map_err(|e| CaptureError::D3DError(format!("Failed to create Direct3D device: {}", e)))?;
        ...
        let frame_pool = Direct3D11CaptureFramePool::CreateFreeThreaded(
            ...
            item_size,
        ).map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to create frame pool: {}", e)))?;
        
        // Create a capture session
        let session = frame_pool.CreateCaptureSession(&item)
            .map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to create capture session: {}", e)))?;
        ...
        match session.SetIsBorderRequired(false) {
            ...
            Err(e) => warn!("⚠️ Could not disable capture border (requires Windows 10 2004+): {}", e),
        }
        ...
        })).map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to subscribe to frame events: {}", e)))?;
        
        // Start capturing
        session.StartCapture()
            .map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to start capture: {}", e)))?;
    ...
    pub fn grab_frame(&mut self) -> Result<(Vec<u8>, u32, u32), CaptureError> {
        ...
        let surface = frame.Surface()
            .map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to get frame surface: {}", e)))?;
        
        // Get the D3D11 texture from the surface
        let source_texture: ID3D11Texture2D = d3d::get_d3d_interface_from_object(&surface)
            .map_err(|e| CaptureError::D3DError(format!("Failed to get texture from surface: {}", e)))?;
    ...
    fn get_or_create_staging_texture(&mut self, source: &ID3D11Texture2D) -> Result<ID3D11Texture2D, CaptureError> {
        ...
        let texture = unsafe {
            ...
            self.d3d_device.CreateTexture2D(&desc, None, Some(&mut texture))
                .map_err(|e| CaptureError::D3DError(format!("Failed to create staging texture: {}", e)))?;
            texture.unwrap()
        };
    ...
    pub fn capture_region(&mut self, region: &CaptureRegion) -> Result<CaptureResult, CaptureError> {
        // Validate the region
        if !region.is_valid() {
...
fn create_capture_item_for_monitor(monitor_handle: HMONITOR) -> Result<GraphicsCaptureItem, CaptureError> {
    let interop = windows::core::factory::<GraphicsCaptureItem, IGraphicsCaptureItemInterop>()
        .map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to get capture interop: {}", e)))?;
    
    unsafe {
        interop.CreateForMonitor(monitor_handle)
            .map_err(|e| CaptureError::GraphicsCaptureError(format!("Failed to create capture item for monitor: {}", e)))
    }
}
