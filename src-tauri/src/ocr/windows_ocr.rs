...
impl WindowsOcr {
    ...
    pub fn with_language(language_tag: &str) -> Result<Self, OcrError> {
        ...
        let language = Language::CreateLanguage(&language_tag.into())
            .map_err(|e| OcrError::LanguageNotSupported(
                format!("Invalid language tag '{}': {}", language_tag, e)
            ))?;
        ...
        {
            return Err(OcrError::LanguageNotSupported(
                format!("Language '{}' is not installed for OCR. Install it in Windows Settings.", language_tag)
            ));
        }
    ...
    pub async fn recognize(
        ...
        debug!("Recognizing text in {}x{} image ({} bytes)", width, height, image_data.len());
        
        // Validate input
        let expected_size = (width * height * 4) as usize;
        if image_data.len() != expected_size {
            return Err(OcrError::InvalidImage(
                format!("Expected {} bytes for {}x{} BGRA image, got {}", 
        ...
        // Run OCR!
        let ocr_result = self.engine.RecognizeAsync(&bitmap)
...
mod tests {
    ...
    fn test_list_available_languages() {
        ...
        assert!(!langs.is_empty(), "At least one OCR language should be installed");
    }
    
    #[test]
    fn test_create_ocr_engine() {
        let ocr = WindowsOcr::new();
        assert!(ocr.is_ok(), "Should be able to create OCR engine: {:?}", ocr.err());
    }
    ...
    async fn test_ocr_empty_image() {
        ...
        assert!(result.is_ok(), "OCR should succeed on blank image: {:?}", result.err());
        
        // A blank white image should have no text
        let ocr_result = result.unwrap();
        assert!(ocr_result.is_empty() || ocr_result.text.trim().is_empty(), 
